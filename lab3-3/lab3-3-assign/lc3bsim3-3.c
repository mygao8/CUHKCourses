...
/*
    *  lab3-3 assignment in the following
    *
    */
...
void latch_datapath_values()
{
    ...
    if (GetLD_MDR(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        ...
        {
            /* insert INMUX here */
            NEXT_LATCHES.MDR = MemOut/*Low16bits((MEMORY[CURRENT_LATCHES.MAR >> 1][1] << 8) + MEMORY[CURRENT_LATCHES.MAR >> 1][0])*/;
        }
        else
        {
            NEXT_LATCHES.MDR = blockMDRLogic2(partVal(CURRENT_LATCHES.MAR, 0, 0), GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION), BUS);
        }
    }
    /* LD.BEN */
    if (GetLD_BEN(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        NEXT_LATCHES.BEN = (partVal(CURRENT_LATCHES.IR, 11, 11) & CURRENT_LATCHES.N) | (partVal(CURRENT_LATCHES.IR, 10, 10) & CURRENT_LATCHES.Z) | (partVal(CURRENT_LATCHES.IR, 9, 9) & CURRENT_LATCHES.P);
    }
    /* LD.REG */
    if (GetLD_REG(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        NEXT_LATCHES.REGS[blockDRMUX(GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION), partVal(CURRENT_LATCHES.IR, 11, 9), 7)] = BUS;
    }
    /*
     *  lab3-3 assignment in the following
     *
    ...
    {
        NEXT_LATCHES.PC = blockPCMUX(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION), CURRENT_LATCHES.PC+2, BUS, inputOfMARMUXFromAdder);
    }
}

...
filename = './class_train_5_3_36k_lux3(254).csv'
dataset = FaceLandmarksDataset(filename,symbol,upsample)
train_loader = torch.utils.data.DataLoader(dataset, batch_size=254, shuffle=True)
epochnum = 40
loss_b = 50
loss_save=[]
for i in range(epochnum):
    for t, data in enumerate(train_loader):
        ...
        if t==0:    
            ...
            if loss.item()<loss_b:
                loss_b = loss.item()
                filename = './modelb_36_lux3(254).pkl'	      
...
filename = './loss_save_36_lux3.csv'
p.to_csv(filename)        
